...
CREATE MASTER KEY ENCRYPTION BY PASSWORD = 'i removed this for secret';
...
WITH (
    LOCATION = 'https://digistoragedatalake.dfs.core.windows.net/silver', -- URL to the external data storage location
    CREDENTIAL = cred_karthik, -- Use the 'cred_karthik' credential for authentication
);

-- Create another external data source named 'source_gold', pointing to the 'gold' directory in Azure Data Lake Storage.
CREATE EXTERNAL DATA SOURCE source_gold
WITH (
    LOCATION = 'https://digistoragedatalake.dfs.core.windows.net/gold', -- URL to the external data storage location
    CREDENTIAL = cred_karthik, -- Use the 'cred_karthik' credential for authentication
);
